...
// public method
//-----------------------------------------------------------------------------------------
void Polygon4::DrawPolygon4Lambert(
	...
	if (polygon2cameraPosDot_ >= 0.0f && polygon2cameraDirectionDot_ < 0.0f) {
		...
		AddPolygonElement(fillMode, blendMode);
	}
}
...
void Polygon4::DrawPolygon4Lambert(
	...
	if (polygon2cameraPosDot_ >= 0.0f && polygon2cameraDirectionDot_ < 0.0f) {
		...
		AddPolygonElement(kSubdivision, texture, textureSize, blendMode);
	}
}
...
void Polygon4::DrawPolygon4BlinnPhong(
	const Vector3f& localPosA, const Vector3f& localPosB, const Vector3f& localPosC, const Vector3f& localPosD,
...
void Polygon4::SetScreenPos(
	...
	Vector3f ndc
		= Matrix4x4::Transform(
			worldCenterPos_, Matrix4x4::MakeAffine({ 1.0f, 1.0f, 1.0f, }, { 0.0f }, { 0.0f }) * camera.GetViewProjectionMatrix()
		);
	...
	} else if (ndcDepth_ < 0.0f) {
		ndcDepth_ = 0.0f;
	}*/
}

void Polygon4::SetCameraDot(
	const Camera3D& camera) {
...
void Polygon4::SetLightAmount(
	...
	switch (light.lightMode_) {
		...
		case LightMode::kLightSpot:
			...
			directionDot
				= Vector::DotProduct(light.GetSpotLightSetting().direction, lightDirection_);
			length = Vector::CalculateLength(worldCenterPos_, light.GetPos());

			if (directionDot >= light.GetSpotLightSetting().angle) {
				if (length < light.GetSpotLightSetting().range) {
					lightAmount_ = std::powf((light.GetSpotLightSetting().range - length) / light.GetSpotLightSetting().range, 2);

					lightAmount_ *= directionDot - Clamp(light.GetSpotLightSetting().angle, 0.0f, 1.0f);
					lightAmount_ = Clamp(lightAmount_, 0.0f, 1.0f);
				}
			}
...
void Polygon4::FixColorLambert(
	...
	for (int ci = 0; ci < 3; ci++) {
		...
		// Fog:
		colorValue[ci] += (ndcDepth_ * 255.0f);

		if (colorValue[ci] < 0.0f) { colorValue[ci] = 0.0f; }
		else if (colorValue[ci] > 255.0f) { colorValue[ci] = 255.0f; }
	}
...
void Polygon4::FixColorBlinnPhong(
...
// manager method
//-----------------------------------------------------------------------------------------
void Polygon4::AddPolygonElement(
	FillMode fillMode, BlendMode blendMode) const {
	SortPolygonElement element = {};

	element.polygonType = PolygonType::kPolygonQuad;
...
void Polygon4::AddPolygonElement(
	uint32_t kSubdivision, int texture, const Vector2f& textureSize, BlendMode blendMode) {

	SortPolygonElement element = {};

	element.polygonType = PolygonType::kPolygonQuadTexture;
	...
	element.fillMode = kFillModeSolid;
	element.blendMode = blendMode;

	element.subdivision = kSubdivision;
